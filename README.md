...
项目会生成全内联样式富文本，并规避常见的不兼容结构。但微信公众号可能根据后台版本或浏览器再次清理内容，正式发布前仍建议人工检查。

### 自定义模板保存在哪里？

目前保存在浏览器的 Local Storage 中。清除浏览器数据会同时删除自定义模板，后续计划支持模板 JSON 导入与导出。

### 可以直接导入 gzh-design-skill 的主题文件吗？

目前不能直接导入 `references/theme-*.md`。Web 模板和原 Skill 组件库使用不同的数据结构，后续可以通过模板转换器进行适配。

### 这个项目会把文章上传到服务器吗？

当前版本的 Markdown 转换、预览和自定义模板均在浏览器本地完成。
...
提交新模板时，请确保输出内容仍符合公众号富文本约束，并保留必要的来源与版权说明。
...
如果这个项目对你有帮助，欢迎点一个 ⭐ Star，也欢迎贡献新的公众号模板。
